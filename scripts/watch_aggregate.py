...
def main() -> int:
    ...
    if current:
        ...
        for item in current:
            ...
            lane = str(item.get("analysis_lane", "FULL_RUNTIME"))
            risk = str(item.get("risk_score", "—"))
            confidence_raw = item.get("confidence")
            confidence = f"{float(confidence_raw) * 100:.0f}%" if isinstance(confidence_raw, (int, float)) else "—"
            ...
            lines.append(
                f"| {repository} | `{short_sha}` | **{result}** | {lane} | {risk} | {confidence} | "
