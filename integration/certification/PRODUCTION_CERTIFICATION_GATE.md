...
v0.9's Adaptive Impact Engine does **not** weaken this boundary. NO_RUNTIME_CHANGE, STATIC_ONLY and FAST_RUNTIME can save continuous-verification work, while FULL_RUNTIME can be selected automatically for risky changes. None of those adaptive lanes can satisfy CERTIFICATION. A certification request always uses the release profile, complete mandatory evidence and the production matrix below.
